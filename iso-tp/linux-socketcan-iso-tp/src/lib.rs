...
compile_error!("linux-socketcan-iso-tp only supports Linux targets");
...
fn poll_fds(fds: &mut [libc::pollfd], timeout: Duration) -> io::Result<usize> {
    let timeout_ms = duration_to_poll_timeout(timeout);
    loop {
...
fn duration_to_nanos_u32(timeout: Duration) -> u32 {
...
fn apply_kernel_options(fd: RawFd, options: &IsoTpKernelOptions) -> Result<(), Error> {
    let opts = build_can_isotp_options(&options.socket);
    set_sockopt(fd, SOL_CAN_ISOTP, CAN_ISOTP_OPTS, &opts)?;

    if let Some(fc) = options.flow_control {
        let fc_opts = can_isotp_fc_options {
            bs: fc.block_size,
            stmin: fc.st_min,
            wftmax: fc.wft_max,
        };
        set_sockopt(fd, SOL_CAN_ISOTP, CAN_ISOTP_RECV_FC, &fc_opts)?;
    }

    if let Some(ll) = options.link_layer {
        let ll_opts = can_isotp_ll_options {
            mtu: ll.mtu,
            tx_dl: ll.tx_dl,
            tx_flags: ll.tx_flags,
        };
        set_sockopt(fd, SOL_CAN_ISOTP, CAN_ISOTP_LL_OPTS, &ll_opts)?;
    }

    if let Some(tx) = options.force_tx_stmin {
        let val = duration_to_nanos_u32(tx);
        set_sockopt(fd, SOL_CAN_ISOTP, CAN_ISOTP_TX_STMIN, &val)?;
    }

    if let Some(rx) = options.force_rx_stmin {
        let val = duration_to_nanos_u32(rx);
        set_sockopt(fd, SOL_CAN_ISOTP, CAN_ISOTP_RX_STMIN, &val)?;
    }

    Ok(())
}

fn set_sockopt<T>(fd: RawFd, level: i32, optname: i32, val: &T) -> Result<(), Error> {
    let res = unsafe {
        libc::setsockopt(
            fd,
            level,
...
fn build_can_isotp_options(options: &IsoTpSocketOptions) -> can_isotp_options {
    let mut flags = options.flags;
    if options.ext_address.is_some() {
        flags |= CAN_ISOTP_EXTEND_ADDR;
    }
    if options.rx_ext_address.is_some() {
        flags |= CAN_ISOTP_RX_EXT_ADDR;
    }
    if options.tx_padding.is_some() {
        flags |= CAN_ISOTP_TX_PADDING;
    }
    if options.rx_padding.is_some() {
        flags |= CAN_ISOTP_RX_PADDING;
    }

    can_isotp_options {
        flags,
        frame_txtime: options.frame_txtime.map(duration_to_nanos_u32).unwrap_or(0),
        ext_address: options.ext_address.unwrap_or(0),
        txpad_content: options.tx_padding.unwrap_or(0),
        rxpad_content: options.rx_padding.unwrap_or(0),
        rx_ext_address: options.rx_ext_address.unwrap_or(0),
    }
}
